...
obten_hipotesis_y_sintomas(Diagnosis, ListaDeSintomas):-
    enfermedad(Diagnosis, ListaDeSintomas).
...
pregunta_sobre(Diagnosis, Sintoma, Reply):- write('Es verdad que tienes '),
    write(Sintoma), write('? '),
...
process(Diagnosis, Sintoma, porque, Reply):- nl,
    ...
    write(Diagnosis), write('.'), nl, write('Para esto necesito saber si tienes '),
...
actua_consecuentemente(Diagnosis, si):- enfermedad(Diagnosis, ListaDeSintomas),
